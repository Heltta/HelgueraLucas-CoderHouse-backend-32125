...
class Container {
    ...
    async #writeObj(obj) {
        ...
        try {
            await promises.writeFile(this.filePath, stringifiedObj);
        } catch (error) {
            logger.log(error);
        }
    }
    ...
    async save(data) {
        ...
        const objId =
            content.length === 0 ? 1 : content[content.length - 1].id + 1;
    ...
    async overwriteById(id, data) {
        ...
        if (content.some((element) => element.id === id)) {
            ...
            const mapedContent = content.map((element) =>
                element.id !== id ? element : { ...data, id: id }
            );
