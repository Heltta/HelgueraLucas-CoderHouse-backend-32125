...
describe('Singleton patron', () => {
    describe.each([
        ...
        test('Properties', () => {
            expect(singletonClass.getInstance).toBeTruthy();
        });
        ...
        test('Output', () => {
            ...
            expect(classInstance).not.toBeNull();
            expect(classInstance).toBeInstanceOf(singletonClass);
            expect(classInstance).toBe(singletonClass.getInstance());
            expect(classInstance.coll).toBeDefined();
            expect(typeof classInstance.coll).toBe('string');
        });
        afterAll(() => {
            singletonClass.disconnectDB();
        }); //prevents openHandles
    });
});
